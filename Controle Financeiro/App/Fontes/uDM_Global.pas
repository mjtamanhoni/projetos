...
begin
  ...
  {$IFDEF ANDROID}
    FDC_Conexao.Params.Values['Database'] := TPath.Combine(TPath.GetDocumentsPath,'FINANCEIRO.S3DB');
  {$ENDIF}
  try
    FDC_Conexao.Connected := True;
  except
    On Ex:Exception do
      raise Exception.Create(TPath.Combine(TPath.GetDocumentsPath,'FINANCEIRO.S3DB') + ' - ' + Ex.Message);
  end;
end;
