...
procedure TSETOR.Criar_Estrutura(const AFDScript: TFDScript; AFDQuery: TFDQuery); 
var 
  lScript: TStringList; 
begin 
  try 
    try 
      ...
      lScript := TStringList.Create; 
       ...
       begin 
         ...
         begin 
           ...
           begin 
            //Excluindo objetos da tabela SETOR... 
 
            if Trigger_Existe(FConexao,AFDQuery,'SETOR_BI') then 
              Add('DROP TRIGGER SETOR_BI;'); 
 
            if Generator_Existe(FConexao,AFDQuery,'GEN_SETOR_ID') then 
              Add('DROP GENERATOR GEN_SETOR_ID;'); 
 
            if Tabela_Existe(FConexao,AFDQuery,'SETOR') then 
              Add('DROP TABLE SETOR;'); 
     ...
     //Criando tabelas...
      lScript.Clear; 
      lScript.Add('CREATE TABLE SETOR ( '); 
      ...
      begin 
        ...
        with SQLScripts[0].SQL do 
        begin 
          Add('ALTER TABLE SETOR ADD CONSTRAINT PK_SETOR PRIMARY KEY (ID);');
          Add('COMMENT ON COLUMN SETOR.ID IS ''SEQUENCIAL'';');
          Add('COMMENT ON COLUMN SETOR.NOME IS ''NOME'';');
          Add('COMMENT ON COLUMN SETOR.ID_USUARIO IS ''CODIGO DO USUARIO'';');
          Add('COMMENT ON COLUMN SETOR.DT_CADASTRO IS ''DATA DO CADASTRO'';');
          Add('COMMENT ON COLUMN SETOR.HR_CADASTRO IS ''HORA DO CADASTRO'';');
          Add('CREATE SEQUENCE GEN_SETOR_ID');
          ValidateAll; 
          ExecuteAll; 
        end; 
      end; 
      ...
      lScript.Add('CREATE OR ALTER TRIGGER SETOR_BI FOR SETOR '); 
      lScript.Add('ACTIVE BEFORE INSERT POSITION 0');
      lScript.Add('as');
      lScript.Add('begin');
      lScript.Add('  if (new.id is null) then');
      lScript.Add('    new.id = gen_id(gen_setor_id,1);');
      lScript.Add('end ^');
      lScript.Add(' '); 
      AFDScript.ExecuteScript(lScript); 
    except 
...
procedure TSETOR.Atualizar_Estrutura(const AFDQ_Query: TFDQuery); 
begin 
 
end; 
 
procedure TSETOR.Inicia_Propriedades; 
begin 
  ID := -1; 
  NOME := ''; 
  ID_USUARIO := -1; 
...
procedure TSETOR.Inserir(const AFDQ_Query: TFDQuery);
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Active := False;
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('INSERT INTO SETOR( ');
...
procedure TSETOR.Atualizar(const AFDQ_Query: TFDQuery; AID:Integer = 0);
begin 
   try 
     try 
       AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Sql.Add('UPDATE SETOR SET ');
      if FNOME <> '' then 
...
procedure TSETOR.Excluir(const AFDQ_Query:TFDQuery; AID:Integer = 0); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('DELETE FROM SETOR '); 
...
procedure TSETOR.SetID( const Value:Integer);
begin 
  FID := Value; 
end;
 
procedure TSETOR.SetNOME( const Value:String);
begin 
  FNOME := Value; 
end;
 
procedure TSETOR.SetID_USUARIO( const Value:Integer);
begin 
  FID_USUARIO := Value; 
end;
 
procedure TSETOR.SetDT_CADASTRO( const Value:TDate);
begin 
  FDT_CADASTRO := Value; 
end;
 
procedure TSETOR.SetHR_CADASTRO( const Value:TTime);
begin 
  FHR_CADASTRO := Value; 
end;
