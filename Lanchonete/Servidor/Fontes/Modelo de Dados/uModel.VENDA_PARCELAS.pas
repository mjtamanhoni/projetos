...
type 
  ...
  public 
    ...
    function Listar(const AFDQ_Query:TFDQuery; AID_VENDA:Integer = 0; AID:Integer = 0): TJSONArray; 
    procedure Atualizar(const AFDQ_Query:TFDQuery; AID_VENDA:Integer = 0; AID:Integer = 0); 
    procedure Excluir(const AFDQ_Query:TFDQuery; AID_VENDA:Integer = 0; AID:Integer = 0); 
...
constructor TVENDA_PARCELAS.Create(AConnexao: TFDConnection); 
begin 
  FConexao := AConnexao; 
end; 
 
destructor TVENDA_PARCELAS.Destroy; 
begin 
 
  inherited; 
end; 
 
procedure TVENDA_PARCELAS.Criar_Estrutura(const AFDScript: TFDScript; AFDQuery: TFDQuery); 
var 
  lScript: TStringList; 
begin 
  try 
    try 
      ...
      lScript := TStringList.Create; 
       ...
       begin 
         ...
         begin 
           ...
           begin 
            //Excluindo objetos da tabela VENDA_PARCELAS... 
            if Indice_Existe(FConexao,AFDQuery,'FK_VENDA_PARCELAS_1') then 
              Add('DROP INDEX FK_VENDA_PARCELAS_1;'); 
            if Indice_Existe(FConexao,AFDQuery,'VENDA_PARCELAS_IDX1') then 
              Add('DROP INDEX VENDA_PARCELAS_IDX1;'); 
            if Indice_Existe(FConexao,AFDQuery,'VENDA_PARCELAS_IDX2') then 
              Add('DROP INDEX VENDA_PARCELAS_IDX2;'); 
            if Indice_Existe(FConexao,AFDQuery,'VENDA_PARCELAS_IDX3') then 
              Add('DROP INDEX VENDA_PARCELAS_IDX3;'); 
 
            if Trigger_Existe(FConexao,AFDQuery,'VENDA_PARCELAS_BI') then 
              Add('DROP TRIGGER VENDA_PARCELAS_BI;'); 
 
 
            if Tabela_Existe(FConexao,AFDQuery,'VENDA_PARCELAS') then 
              Add('DROP TABLE VENDA_PARCELAS;'); 
     ...
     //Criando tabelas...
      lScript.Clear; 
      lScript.Add('CREATE TABLE VENDA_PARCELAS ( '); 
      lScript.Add('  ID_VENDA INTEGER NOT NULL');
      ...
      begin 
        ...
        with SQLScripts[0].SQL do 
        begin 
          Add('ALTER TABLE VENDA_PARCELAS ADD CONSTRAINT PK_VENDA_PARCELAS PRIMARY KEY (ID_VENDA,ID);');
          Add('ALTER TABLE VENDA_PARCELAS ADD CONSTRAINT FK_VENDA_PARCELAS_1 FOREIGN KEY (ID_VENDA) REFERENCES VENDA(ID) ON DELETE CASCADE;');
          Add('CREATE INDEX FK_VENDA_PARCELAS_1 ON VENDA_PARCELAS (ID_VENDA);');
          Add('CREATE INDEX VENDA_PARCELAS_IDX1 ON VENDA_PARCELAS (STATUS);');
          Add('CREATE INDEX VENDA_PARCELAS_IDX2 ON VENDA_PARCELAS (DT_VENCIMENTO);');
          Add('CREATE INDEX VENDA_PARCELAS_IDX3 ON VENDA_PARCELAS (DT_PAGO);');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.ID_VENDA IS ''CODIGO DA VENDA'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.ID IS ''SEQUENCIAL'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.PARCELA IS ''CODIGO DA PARCELA'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.DT_VENCIMENTO IS ''DATA DO VENCIMENTO'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.VLR_PARCELA IS ''VARLO DA PARCELA'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.STATUS IS ''STATUS: 0-ABERTO, 1-PAGO'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.ACRESCIMO IS ''ACRESCIMO'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.DESCONTO IS ''DESCONTO'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.DT_PAGO IS ''DATA DO PAGAMENTO'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.HR_PAGO IS ''HORA DO PAGAMENTO'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.VLR_PAGO IS ''VALOR PAGO'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.ID_USUARIO IS ''CODIGO DO USUARIO'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.DT_CADASTRO IS ''DATA DO CADASTRO'';');
          Add('COMMENT ON COLUMN VENDA_PARCELAS.HR_CADASTRO IS ''HORA DO CADASTRO'';');
          ValidateAll; 
          ExecuteAll; 
        end; 
      end; 
      ...
      lScript.Add('CREATE OR ALTER TRIGGER VENDA_PARCELAS_BI FOR VENDA_PARCELAS '); 
      lScript.Add('ACTIVE BEFORE INSERT POSITION 0');
      lScript.Add('AS');
      lScript.Add('BEGIN');
      lScript.Add('  IF (NEW.ID IS NULL) THEN');
      lScript.Add('    SELECT COALESCE(MAX(ID),0) + 1 FROM VENDA_PARCELAS WHERE ID_VENDA = NEW.ID_VENDA INTO NEW.ID;');
      lScript.Add('END ^');
      lScript.Add(' '); 
      AFDScript.ExecuteScript(lScript); 
    except 
...
procedure TVENDA_PARCELAS.Atualizar_Estrutura(const AFDQ_Query: TFDQuery); 
begin 
 
end; 
 
procedure TVENDA_PARCELAS.Inicia_Propriedades; 
begin 
  ID_VENDA := -1; 
  ID := -1; 
  PARCELA := -1; 
...
function  TVENDA_PARCELAS.Listar(const AFDQ_Query:TFDQuery; AID_VENDA:Integer = 0; AID:Integer = 0): TJSONArray; 
begin 
  try 
    try 
      ...
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('SELECT * FROM VENDA_PARCELAS') ;
      AFDQ_Query.Sql.Add('WHERE 1=1');
      AFDQ_Query.Sql.Add('  AND ID_VENDA = :ID_VENDA');
      AFDQ_Query.ParamByName('ID_VENDA').AsInteger := AID_VENDA;
      ...
      begin 
        ID_VENDA := AFDQ_Query.FieldByName('ID_VENDA').AsInteger;
...
procedure TVENDA_PARCELAS.Inserir(const AFDQ_Query: TFDQuery); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('INSERT INTO VENDA_PARCELAS( ');
      AFDQ_Query.Sql.Add('  ID_VENDA');
      ...
      AFDQ_Query.Sql.Add('  ,:HR_CADASTRO');
      AFDQ_Query.Sql.Add(');');
      AFDQ_Query.ParamByName('ID_VENDA').AsInteger := FID_VENDA;
...
procedure TVENDA_PARCELAS.Atualizar(const AFDQ_Query: TFDQuery; AID_VENDA:Integer = 0; AID:Integer = 0); 
begin 
   try 
     try 
       AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Sql.Add('UPDATE VENDA_PARCELAS SET ');
      if FPARCELA > -1 then 
...
procedure TVENDA_PARCELAS.Excluir(const AFDQ_Query:TFDQuery; AID_VENDA:Integer = 0; AID:Integer = 0); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('DELETE FROM VENDA_PARCELAS '); 
...
procedure TVENDA_PARCELAS.SetID_VENDA( const Value:Integer);
begin 
  FID_VENDA := Value; 
end;
 
procedure TVENDA_PARCELAS.SetID( const Value:Integer);
begin 
  FID := Value; 
end;
 
procedure TVENDA_PARCELAS.SetPARCELA( const Value:Integer);
begin 
  FPARCELA := Value; 
end;
 
procedure TVENDA_PARCELAS.SetDT_VENCIMENTO( const Value:TDate);
begin 
  FDT_VENCIMENTO := Value; 
end;
 
procedure TVENDA_PARCELAS.SetVLR_PARCELA( const Value:Double);
begin 
  FVLR_PARCELA := Value; 
end;
 
procedure TVENDA_PARCELAS.SetSTATUS( const Value:Integer);
begin 
  FSTATUS := Value; 
end;
 
procedure TVENDA_PARCELAS.SetACRESCIMO( const Value:Double);
begin 
  FACRESCIMO := Value; 
end;
 
procedure TVENDA_PARCELAS.SetDESCONTO( const Value:Double);
begin 
  FDESCONTO := Value; 
end;
 
procedure TVENDA_PARCELAS.SetDT_PAGO( const Value:TDate);
begin 
  FDT_PAGO := Value; 
end;
 
procedure TVENDA_PARCELAS.SetHR_PAGO( const Value:TTime);
begin 
  FHR_PAGO := Value; 
end;
 
procedure TVENDA_PARCELAS.SetVLR_PAGO( const Value:Double);
begin 
  FVLR_PAGO := Value; 
end;
 
procedure TVENDA_PARCELAS.SetID_USUARIO( const Value:Integer);
begin 
  FID_USUARIO := Value; 
end;
 
procedure TVENDA_PARCELAS.SetDT_CADASTRO( const Value:TDate);
begin 
  FDT_CADASTRO := Value; 
end;
 
procedure TVENDA_PARCELAS.SetHR_CADASTRO( const Value:TTime);
begin 
  FHR_CADASTRO := Value; 
end;
