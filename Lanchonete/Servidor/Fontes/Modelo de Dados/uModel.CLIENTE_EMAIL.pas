...
type 
  ...
  public 
    ...
    function Listar(const AFDQ_Query:TFDQuery; AID_CLIENTE:Integer = 0; AID:Integer = 0; APagina:Integer=0): TJSONArray; 
    procedure Atualizar(const AFDQ_Query:TFDQuery; AID_CLIENTE:Integer = 0; AID:Integer = 0);
...
constructor TCLIENTE_EMAIL.Create(AConnexao: TFDConnection); 
begin 
  FPaginas := C_Paginas; 
  FConexao := AConnexao; 
end; 
 
destructor TCLIENTE_EMAIL.Destroy; 
begin 
 
  inherited; 
end; 
 
procedure TCLIENTE_EMAIL.Criar_Estrutura(const AFDScript: TFDScript; AFDQuery: TFDQuery); 
var 
  lScript: TStringList; 
begin 
  try 
    try 
      ...
      lScript := TStringList.Create; 
       ...
       begin 
         ...
         begin 
           ...
           begin 
            //Excluindo objetos da tabela CLIENTE_EMAIL... 
            if Indice_Existe(FConexao,AFDQuery,'FK_CLIENTE_EMAIL_1') then 
              Add('DROP INDEX FK_CLIENTE_EMAIL_1;'); 
            if Indice_Existe(FConexao,AFDQuery,'FK_CLIENTE_EMAIL_2') then 
              Add('DROP INDEX FK_CLIENTE_EMAIL_2;'); 
 
            if Trigger_Existe(FConexao,AFDQuery,'CLIENTE_EMAIL_BI') then 
              Add('DROP TRIGGER CLIENTE_EMAIL_BI;'); 
 
 
            if Tabela_Existe(FConexao,AFDQuery,'CLIENTE_EMAIL') then 
              Add('DROP TABLE CLIENTE_EMAIL;'); 
     ...
     //Criando tabelas...
      lScript.Clear; 
      lScript.Add('CREATE TABLE CLIENTE_EMAIL ( '); 
      ...
      begin 
        ...
        with SQLScripts[0].SQL do 
        begin 
          Add('ALTER TABLE CLIENTE_EMAIL ADD CONSTRAINT PK_CLIENTE_EMAIL PRIMARY KEY (ID_CLIENTE,ID);');
          Add('ALTER TABLE CLIENTE_EMAIL ADD CONSTRAINT FK_CLIENTE_EMAIL_1 FOREIGN KEY (ID_CLIENTE) REFERENCES CLIENTE(ID) ON DELETE CASCADE;');
          Add('ALTER TABLE CLIENTE_EMAIL ADD CONSTRAINT FK_CLIENTE_EMAIL_2 FOREIGN KEY (ID_SETOR) REFERENCES SETOR(ID) ;');
          Add('CREATE INDEX FK_CLIENTE_EMAIL_1 ON CLIENTE_EMAIL (ID_CLIENTE);');
          Add('CREATE INDEX FK_CLIENTE_EMAIL_2 ON CLIENTE_EMAIL (ID_SETOR);');
          Add('COMMENT ON COLUMN CLIENTE_EMAIL.ID_CLIENTE IS ''CODIGO DO CLIENTE'';');
          Add('COMMENT ON COLUMN CLIENTE_EMAIL.ID IS ''SEQUENCIAL'';');
          Add('COMMENT ON COLUMN CLIENTE_EMAIL.RESPONSAVEL IS ''NOME DO RESPONSAVEL PELO EMAIL'';');
          Add('COMMENT ON COLUMN CLIENTE_EMAIL.ID_SETOR IS ''CODIGO DO SETOR'';');
          Add('COMMENT ON COLUMN CLIENTE_EMAIL.EMAIL IS ''EMAIL'';');
          Add('COMMENT ON COLUMN CLIENTE_EMAIL.ID_USUARIO IS ''CODIGO DO USUARIO'';');
          Add('COMMENT ON COLUMN CLIENTE_EMAIL.DT_CADASTRO IS ''DATA DO CADASTRO'';');
          Add('COMMENT ON COLUMN CLIENTE_EMAIL.HF_CADASTRO IS ''HORA DO CADASTRO'';');
          ValidateAll; 
          ExecuteAll; 
        end; 
      end; 
      ...
      lScript.Add('CREATE OR ALTER TRIGGER CLIENTE_EMAIL_BI FOR CLIENTE_EMAIL '); 
      lScript.Add('ACTIVE BEFORE INSERT POSITION 0');
      lScript.Add('AS');
      lScript.Add('BEGIN');
      lScript.Add('  IF (NEW.ID IS NULL) THEN');
      lScript.Add('    SELECT COALESCE(MAX(ID),0) + 1 FROM CLIENTE_EMAIL WHERE ID_CLIENTE = NEW.ID_CLIENTE INTO NEW.ID;');
      lScript.Add('END ^');
      lScript.Add(' '); 
      AFDScript.ExecuteScript(lScript); 
    except 
...
procedure TCLIENTE_EMAIL.Atualizar_Estrutura(const AFDQ_Query: TFDQuery); 
begin 
 
end; 
 
procedure TCLIENTE_EMAIL.Inicia_Propriedades; 
begin 
  ID_CLIENTE := -1; 
  ID := -1; 
  RESPONSAVEL := ''; 
...
function  TCLIENTE_EMAIL.Listar(const AFDQ_Query:TFDQuery; AID_CLIENTE:Integer = 0; AID:Integer = 0; APagina:Integer=0): TJSONArray; 
begin 
  try 
    try 
      ...
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('SELECT * FROM CLIENTE_EMAIL') ;
...
procedure TCLIENTE_EMAIL.Inserir(const AFDQ_Query: TFDQuery); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('INSERT INTO CLIENTE_EMAIL( ');
...
procedure TCLIENTE_EMAIL.Atualizar(const AFDQ_Query: TFDQuery; AID_CLIENTE:Integer = 0; AID:Integer = 0); 
begin 
   try 
     try 
       AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Sql.Add('UPDATE CLIENTE_EMAIL SET ');
...
procedure TCLIENTE_EMAIL.Excluir(const AFDQ_Query:TFDQuery; AID_CLIENTE:Integer = 0; AID:Integer = 0); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('DELETE FROM CLIENTE_EMAIL '); 
...
procedure TCLIENTE_EMAIL.SetID_CLIENTE( const Value:Integer);
begin 
  FID_CLIENTE := Value; 
end;
 
procedure TCLIENTE_EMAIL.SetID( const Value:Integer);
begin 
  FID := Value; 
end;
 
procedure TCLIENTE_EMAIL.SetRESPONSAVEL( const Value:String);
begin 
  FRESPONSAVEL := Value; 
end;
 
procedure TCLIENTE_EMAIL.SetID_SETOR( const Value:Integer);
begin 
  FID_SETOR := Value; 
end;
 
procedure TCLIENTE_EMAIL.SetEMAIL( const Value:String);
begin 
  FEMAIL := Value; 
end;
 
procedure TCLIENTE_EMAIL.SetID_USUARIO( const Value:Integer);
begin 
  FID_USUARIO := Value; 
end;
 
procedure TCLIENTE_EMAIL.SetDT_CADASTRO( const Value:TDate);
begin 
  FDT_CADASTRO := Value; 
end;
 
procedure TCLIENTE_EMAIL.SetHF_CADASTRO( const Value:TTime);
begin 
  FHF_CADASTRO := Value; 
end;
