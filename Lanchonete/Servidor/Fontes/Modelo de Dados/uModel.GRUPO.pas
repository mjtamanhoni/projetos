...
constructor TGRUPO.Create(AConnexao: TFDConnection); 
begin 
  FConexao := AConnexao; 
end; 
...
procedure TGRUPO.Criar_Estrutura(const AFDScript: TFDScript; AFDQuery: TFDQuery); 
var 
  lScript: TStringList; 
begin 
  try 
    try 
      ...
      lScript := TStringList.Create; 
       ...
       begin 
         ...
         begin 
           ...
           begin 
            //Excluindo objetos da tabela GRUPO... 
 
            if Trigger_Existe(FConexao,AFDQuery,'GRUPO_BI') then 
              Add('DROP TRIGGER GRUPO_BI;'); 
 
            if Generator_Existe(FConexao,AFDQuery,'GEN_GRUPO_ID') then 
              Add('DROP GENERATOR GEN_GRUPO_ID;'); 
 
            if Tabela_Existe(FConexao,AFDQuery,'GRUPO') then 
              Add('DROP TABLE GRUPO;'); 
     ...
     //Criando tabelas...
      lScript.Clear; 
      lScript.Add('CREATE TABLE GRUPO ( '); 
      ...
      begin 
        ...
        with SQLScripts[0].SQL do 
        begin 
          Add('ALTER TABLE GRUPO ADD CONSTRAINT PK_GRUPO PRIMARY KEY (ID);');
          Add('COMMENT ON COLUMN GRUPO.ID IS ''SEQUENCIAL'';');
          Add('COMMENT ON COLUMN GRUPO.NOME IS ''NOME'';');
          Add('COMMENT ON COLUMN GRUPO.ID_USUARIO IS ''CODIGO DO USUARIO'';');
          Add('COMMENT ON COLUMN GRUPO.DT_CADASTRO IS ''DATA DO CADASTRO'';');
          Add('COMMENT ON COLUMN GRUPO.HR_CADASTRO IS ''HORA DO CADASTRO'';');
          Add('CREATE SEQUENCE GEN_GRUPO_ID');
          ValidateAll; 
          ExecuteAll; 
        end; 
      end; 
      ...
      lScript.Add('CREATE OR ALTER TRIGGER GRUPO_BI FOR GRUPO '); 
      lScript.Add('ACTIVE BEFORE INSERT POSITION 0');
      lScript.Add('as');
      lScript.Add('begin');
      lScript.Add('  if (new.id is null) then');
      lScript.Add('    new.id = gen_id(gen_grupo_id,1);');
      lScript.Add('end ^');
      lScript.Add(' '); 
      AFDScript.ExecuteScript(lScript); 
    except 
...
procedure TGRUPO.Atualizar_Estrutura(const AFDQ_Query: TFDQuery); 
begin 
 
end; 
 
procedure TGRUPO.Inicia_Propriedades; 
begin 
  ID := -1; 
  NOME := ''; 
  ID_USUARIO := -1; 
...
function  TGRUPO.Listar(const AFDQ_Query:TFDQuery; AID:Integer = 0): TJSONArray;
begin
  try
    try
      ...
      AFDQ_Query.Active := False;
      AFDQ_Query.Sql.Clear;
      AFDQ_Query.Sql.Add('SELECT * FROM GRUPO');
...
procedure TGRUPO.Inserir(const AFDQ_Query: TFDQuery); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('INSERT INTO GRUPO( ');
...
procedure TGRUPO.Atualizar(const AFDQ_Query: TFDQuery; AID:Integer = 0); 
begin 
   try 
     try 
       AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Sql.Add('UPDATE GRUPO SET ');
      if FNOME <> '' then 
...
procedure TGRUPO.Excluir(const AFDQ_Query:TFDQuery; AID:Integer = 0); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('DELETE FROM GRUPO '); 
      AFDQ_Query.Sql.Add('WHERE NOT ID IS NULL '); 
      AFDQ_Query.Sql.Add('  AND ID = :ID');
...
procedure TGRUPO.SetID_USUARIO( const Value:Integer);
begin 
  FID_USUARIO := Value; 
end;
 
procedure TGRUPO.SetDT_CADASTRO( const Value:TDate);
begin 
  FDT_CADASTRO := Value; 
end;
 
procedure TGRUPO.SetHR_CADASTRO( const Value:TTime);
begin 
  FHR_CADASTRO := Value; 
end;
