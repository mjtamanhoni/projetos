...
constructor TFORMA_PAGAMENTO.Create(AConnexao: TFDConnection); 
begin 
  FConexao := AConnexao; 
end; 
...
procedure TFORMA_PAGAMENTO.Criar_Estrutura(const AFDScript: TFDScript; AFDQuery: TFDQuery); 
var 
  lScript: TStringList; 
begin 
  try 
    try 
      ...
      lScript := TStringList.Create; 
       ...
       begin 
         ...
         begin 
           ...
           begin 
            //Excluindo objetos da tabela FORMA_PAGAMENTO... 
 
            if Trigger_Existe(FConexao,AFDQuery,'FORMA_PAGAMENTO_BI') then 
              Add('DROP TRIGGER FORMA_PAGAMENTO_BI;'); 
 
            if Generator_Existe(FConexao,AFDQuery,'GEN_FORMA_PAGAMENTO_ID') then 
              Add('DROP GENERATOR GEN_FORMA_PAGAMENTO_ID;'); 
 
            if Tabela_Existe(FConexao,AFDQuery,'FORMA_PAGAMENTO') then 
              Add('DROP TABLE FORMA_PAGAMENTO;'); 
     ...
     //Criando tabelas...
      lScript.Clear; 
      lScript.Add('CREATE TABLE FORMA_PAGAMENTO ( '); 
      ...
      begin 
        ...
        with SQLScripts[0].SQL do 
        begin 
          Add('ALTER TABLE FORMA_PAGAMENTO ADD CONSTRAINT PK_FORMA_PAGAMENTO PRIMARY KEY (ID);');
          Add('COMMENT ON COLUMN FORMA_PAGAMENTO.ID IS ''SEQUENCIAL'';');
          Add('COMMENT ON COLUMN FORMA_PAGAMENTO.NOME IS ''NOME'';');
          Add('COMMENT ON COLUMN FORMA_PAGAMENTO.ID_USUARIO IS ''CODIGO DO USUARIO'';');
          Add('COMMENT ON COLUMN FORMA_PAGAMENTO.DT_CADASTRO IS ''DATA DO CADASTRO'';');
          Add('COMMENT ON COLUMN FORMA_PAGAMENTO.HR_CADASTRO IS ''HORA DO CADASTRO'';');
          Add('CREATE SEQUENCE GEN_FORMA_PAGAMENTO_ID');
          ValidateAll; 
          ExecuteAll; 
        end; 
      end; 
      ...
      lScript.Add('CREATE OR ALTER TRIGGER FORMA_PAGAMENTO_BI FOR FORMA_PAGAMENTO '); 
      lScript.Add('ACTIVE BEFORE INSERT POSITION 0');
      lScript.Add('as');
      lScript.Add('begin');
      lScript.Add('  if (new.id is null) then');
      lScript.Add('    new.id = gen_id(gen_forma_pagamento_id,1);');
      lScript.Add('end ^');
      lScript.Add(' '); 
      AFDScript.ExecuteScript(lScript); 
    except 
...
procedure TFORMA_PAGAMENTO.Atualizar_Estrutura(const AFDQ_Query: TFDQuery); 
begin 
 
end; 
 
procedure TFORMA_PAGAMENTO.Inicia_Propriedades; 
begin 
  ID := -1; 
  NOME := ''; 
  ID_USUARIO := -1; 
...
function  TFORMA_PAGAMENTO.Listar(const AFDQ_Query:TFDQuery; AID:Integer = 0): TJSONArray; 
begin 
  try 
    try 
      ...
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('SELECT * FROM FORMA_PAGAMENTO') ;
...
procedure TFORMA_PAGAMENTO.Inserir(const AFDQ_Query: TFDQuery); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('INSERT INTO FORMA_PAGAMENTO( ');
...
procedure TFORMA_PAGAMENTO.Atualizar(const AFDQ_Query: TFDQuery; AID:Integer = 0); 
begin 
   try 
     try 
       AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Sql.Add('UPDATE FORMA_PAGAMENTO SET ');
      if FNOME <> '' then 
...
procedure TFORMA_PAGAMENTO.Excluir(const AFDQ_Query:TFDQuery; AID:Integer = 0); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('DELETE FROM FORMA_PAGAMENTO '); 
...
procedure TFORMA_PAGAMENTO.SetNOME( const Value:String);
begin 
  FNOME := Value; 
end;
 
procedure TFORMA_PAGAMENTO.SetID_USUARIO( const Value:Integer);
begin 
  FID_USUARIO := Value; 
end;
 
procedure TFORMA_PAGAMENTO.SetDT_CADASTRO( const Value:TDate);
begin 
  FDT_CADASTRO := Value; 
end;
 
procedure TFORMA_PAGAMENTO.SetHR_CADASTRO( const Value:TTime);
begin 
  FHR_CADASTRO := Value; 
end;
