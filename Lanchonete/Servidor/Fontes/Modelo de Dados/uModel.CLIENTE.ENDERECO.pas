...
procedure TCLIENTE_ENDERECO.Criar_Estrutura(const AFDScript: TFDScript; AFDQuery: TFDQuery);
var
  lScript: TStringList;
begin
  try
       ...
       begin
         ...
         begin
           ...
           begin
            ...
            if Indice_Existe(FConexao,AFDQuery,'FK_CLIENTE_ENDERECO_1') then
              Add('DROP INDEX FK_CLIENTE_ENDERECO_1;');
     ...
     //Criando tabelas...
      ...
      begin
        ...
        begin
          ...
          Add('CREATE INDEX FK_CLIENTE_ENDERECO_1 ON CLIENTE_ENDERECO (ID_CLIENTE);');
          ...
          Add('COMMENT ON COLUMN CLIENTE_ENDERECO.CEP IS ''CODIGO DO ENDERECO POSTAL'';');
          ...
          Add('COMMENT ON COLUMN CLIENTE_ENDERECO.IBGE IS ''CÃ“DIGO IBGE DO MUNICÃPIO'';');
          Add('COMMENT ON COLUMN CLIENTE_ENDERECO.MUNICIPIO IS ''NOME DO MUNICÃPIO'';');
          ...
          Add('COMMENT ON COLUMN CLIENTE_ENDERECO.REGIAO IS ''NOME DA REGIÃƒO NO PAIS'';');
          Add('COMMENT ON COLUMN CLIENTE_ENDERECO.CODIGO_PAIS IS ''CODIGO INTERNACIONAL DO PAIS'';');
          Add('COMMENT ON COLUMN CLIENTE_ENDERECO.PAIS IS ''NOME DO PAIS'';');
          Add('COMMENT ON COLUMN CLIENTE_ENDERECO.ID_USUARIO IS ''CODIGO DO USUÃRIO'';');
...
begin
  try
    try
      ...
      AFDQ_Query.Active := False;
      AFDQ_Query.Sql.Clear;
      AFDQ_Query.Sql.Add('SELECT * FROM CLIENTE_ENDERECO CE');
      AFDQ_Query.Sql.Add('WHERE NOT CE.ID_CLIENTE IS NULL');
  ...
  try
    try
      ...
      if FID_CLIENTE = 0 then
        raise Exception.Create('ID_CLIENTE: CODIGO DA CLIENTE não informado');
...
procedure TCLIENTE_ENDERECO.Atualizar(const AFDQ_Query: TFDQuery; AID_CLIENTE:Integer = 0; AID:Integer = 0);
begin
   try
     try
       AFDQ_Query.Connection := FConexao;
...
procedure TCLIENTE_ENDERECO.Excluir(const AFDQ_Query:TFDQuery; AID_CLIENTE:Integer = 0; AID:Integer = 0);
begin
  try
    try
      AFDQ_Query.Connection := FConexao;
