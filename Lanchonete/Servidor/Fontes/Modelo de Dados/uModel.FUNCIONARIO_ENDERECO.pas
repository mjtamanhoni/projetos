unit uModel.FUNCIONARIO_ENDERECO; 
...
{ TFUNCIONARIO_ENDERECO }
 
constructor TFUNCIONARIO_ENDERECO.Create(AConnexao: TFDConnection); 
begin 
  FConexao := AConnexao; 
end; 
 
destructor TFUNCIONARIO_ENDERECO.Destroy; 
begin 
 
  inherited; 
end; 
 
procedure TFUNCIONARIO_ENDERECO.Criar_Estrutura(const AFDScript: TFDScript; AFDQuery: TFDQuery); 
var 
  lScript: TStringList; 
begin 
  try 
    try 
      ...
      lScript := TStringList.Create; 
       ...
       begin 
         ...
         begin 
           ...
           begin 
            //Excluindo objetos da tabela FUNCIONARIO_ENDERECO... 
            if Indice_Existe(FConexao,AFDQuery,'FK_FUNCIONARIO_ENDERECO_1') then 
              Add('DROP INDEX FK_FUNCIONARIO_ENDERECO_1;'); 
 
            if Trigger_Existe(FConexao,AFDQuery,'FUNCIONARIO_ENDERECO_BI') then 
              Add('DROP TRIGGER FUNCIONARIO_ENDERECO_BI;'); 
 
 
            if Tabela_Existe(FConexao,AFDQuery,'FUNCIONARIO_ENDERECO') then 
              Add('DROP TABLE FUNCIONARIO_ENDERECO;'); 
     ...
     //Criando tabelas...
      lScript.Clear; 
      lScript.Add('CREATE TABLE FUNCIONARIO_ENDERECO ( '); 
      ...
      begin 
        ...
        with SQLScripts[0].SQL do 
        begin 
          Add('ALTER TABLE FUNCIONARIO_ENDERECO ADD CONSTRAINT PK_FUNCIONARIO_ENDERECO PRIMARY KEY (ID_FUNCIONARIO,ID);');
          Add('ALTER TABLE FUNCIONARIO_ENDERECO ADD CONSTRAINT FK_FUNCIONARIO_ENDERECO_1 FOREIGN KEY (ID_FUNCIONARIO) REFERENCES FUNCIONARIO(ID) ON DELETE CASCADE;');
          Add('CREATE INDEX FK_FUNCIONARIO_ENDERECO_1 ON FUNCIONARIO_ENDERECO (ID_FUNCIONARIO);');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.ID_FUNCIONARIO IS ''CODIGO DO FUNCIONARIO'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.ID IS ''SEQUENCIAL'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.CEP IS ''CÓDIGO DO ENDEREÇO POSTAL'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.LOGRADOURO IS ''LOGRADOURO: RUA, AVENIDA, BECO...'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.NUMERO IS ''NUMERO DO ESTABELECIMENTO'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.COMPLEMENTO IS ''COMPLEMENTO'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.BAIRRO IS ''BAIRRO'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.IBGE IS ''CÓDIGO IBGE DO MUNICÍPIO'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.MUNICIPIO IS ''NOME DO MUNICÍPIO'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.SIGLA_UF IS ''SIGLA DA UNIDADE FEDERATIVA'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.UF IS ''NOME DA UNIDADE FEDERATIVA'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.REGIAO IS ''NOME DA REGIÃO NO PAIS'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.CODIGO_PAIS IS ''CÓDIGO INTERNACIONAL DO PAIS'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.PAIS IS ''NOME DO PAIS'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.ID_USUARIO IS ''CODIGO DO USUÁRIO'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.DT_CADASTRO IS ''DATA DO CADASTRO'';');
          Add('COMMENT ON COLUMN FUNCIONARIO_ENDERECO.HR_CADASTRO IS ''HORA DO CADASTRO'';');
          ValidateAll; 
          ExecuteAll; 
        end; 
      end; 
      ...
      lScript.Add('CREATE OR ALTER TRIGGER FUNCIONARIO_ENDERECO_BI FOR FUNCIONARIO_ENDERECO '); 
      lScript.Add('ACTIVE BEFORE INSERT POSITION 0');
      lScript.Add('AS');
      lScript.Add('BEGIN');
      lScript.Add('  IF (NEW.ID IS NULL) THEN');
      lScript.Add('    SELECT COALESCE(MAX(ID),0) + 1 FROM FUNCIONARIO_ENDERECO WHERE ID_FUNCIONARIO = NEW.ID_FUNCIONARIO INTO NEW.ID;');
      lScript.Add('END ^');
      lScript.Add(' '); 
      AFDScript.ExecuteScript(lScript); 
    except 
...
procedure TFUNCIONARIO_ENDERECO.Atualizar_Estrutura(const AFDQ_Query: TFDQuery); 
begin 
 
end; 
 
procedure TFUNCIONARIO_ENDERECO.Inicia_Propriedades; 
begin 
  ID_FUNCIONARIO := -1; 
  ID := -1; 
  CEP := ''; 
...
function  TFUNCIONARIO_ENDERECO.Listar(const AFDQ_Query:TFDQuery; AID_FUNCIONARIO:Integer = 0; AID:Integer = 0): TJSONArray; 
begin 
  try 
    try 
      ...
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('SELECT * FROM FUNCIONARIO_ENDERECO') ;
...
procedure TFUNCIONARIO_ENDERECO.Inserir(const AFDQ_Query: TFDQuery); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('INSERT INTO FUNCIONARIO_ENDERECO( ');
...
procedure TFUNCIONARIO_ENDERECO.Atualizar(const AFDQ_Query: TFDQuery; AID_FUNCIONARIO:Integer = 0; AID:Integer = 0); 
begin 
   try 
     try 
       AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Sql.Add('UPDATE FUNCIONARIO_ENDERECO SET ');
      if FCEP <> '' then 
...
procedure TFUNCIONARIO_ENDERECO.Excluir(const AFDQ_Query:TFDQuery; AID_FUNCIONARIO:Integer = 0; AID:Integer = 0); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('DELETE FROM FUNCIONARIO_ENDERECO '); 
...
procedure TFUNCIONARIO_ENDERECO.SetID_FUNCIONARIO( const Value:Integer);
begin 
  FID_FUNCIONARIO := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetID( const Value:Integer);
begin 
  FID := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetCEP( const Value:String);
begin 
  FCEP := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetLOGRADOURO( const Value:String);
begin 
  FLOGRADOURO := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetNUMERO( const Value:String);
begin 
  FNUMERO := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetCOMPLEMENTO( const Value:String);
begin 
  FCOMPLEMENTO := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetBAIRRO( const Value:String);
begin 
  FBAIRRO := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetIBGE( const Value:Integer);
begin 
  FIBGE := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetMUNICIPIO( const Value:String);
begin 
  FMUNICIPIO := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetSIGLA_UF( const Value:String);
begin 
  FSIGLA_UF := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetUF( const Value:String);
begin 
  FUF := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetREGIAO( const Value:String);
begin 
  FREGIAO := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetCODIGO_PAIS( const Value:Integer);
begin 
  FCODIGO_PAIS := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetPAIS( const Value:String);
begin 
  FPAIS := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetID_USUARIO( const Value:Integer);
begin 
  FID_USUARIO := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetDT_CADASTRO( const Value:TDate);
begin 
  FDT_CADASTRO := Value; 
end;
 
procedure TFUNCIONARIO_ENDERECO.SetHR_CADASTRO( const Value:TTime);
begin 
  FHR_CADASTRO := Value; 
end;
