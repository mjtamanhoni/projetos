...
constructor TCLIENTE_TELEFONES.Create(AConnexao: TFDConnection);
begin
  FPaginas := C_Paginas;
  FConexao := AConnexao;
end;

destructor TCLIENTE_TELEFONES.Destroy;
begin

  inherited;
end;

procedure TCLIENTE_TELEFONES.Criar_Estrutura(const AFDScript: TFDScript; AFDQuery: TFDQuery);
var
  lScript: TStringList;
begin
  try
      ...
      lScript := TStringList.Create;
       ...
       begin
         ...
         begin
           ...
           begin
            //Excluindo objetos da tabela CLIENTE_TELEFONES...
            if Indice_Existe(FConexao,AFDQuery,'FK_CLIENTE_TELEFONES_1') then
              Add('DROP INDEX FK_CLIENTE_TELEFONES_1;');
     ...
     //Criando tabelas...
      ...
      begin
        ...
        begin
          ...
          Add('CREATE INDEX FK_CLIENTE_TELEFONES_1 ON CLIENTE_TELEFONES (ID_CLIENTE);');
          Add('COMMENT ON COLUMN CLIENTE_TELEFONES.ID_CLIENTE IS ''CÃ“DIGO DO CLIENTE'';');
          ...
          Add('COMMENT ON COLUMN CLIENTE_TELEFONES.ID_USUARIO IS ''CÃ“DIGO DO USUÃRIO'';');
...
procedure TCLIENTE_TELEFONES.Atualizar_Estrutura(const AFDQ_Query: TFDQuery);
begin

end;

procedure TCLIENTE_TELEFONES.Inicia_Propriedades;
begin
  ID_CLIENTE := -1;
  ID := -1;
  TIPO := -1;
...
function  TCLIENTE_TELEFONES.Listar(
  const AFDQ_Query:TFDQuery;
  const AID_CLIENTE:Integer = 0;
  const AID:Integer = 0;
  const APagina:Integer=0): TJSONArray;
...
procedure TCLIENTE_TELEFONES.Inserir(const AFDQ_Query: TFDQuery);
begin
  try
    try
      AFDQ_Query.Connection := FConexao;
      ...
      if FTIPO = -1 then
        raise Exception.Create('TIPO: TIPO DO CLIENTE: 0-TELEFONE COMERCIAL, 1-CELULAR, 2-TELEFONE RESIDENCIAL não informado');
      if FNUMERO = '' then
      ...
      if FID_CLIENTE = 0 then
        raise Exception.Create('FID_CLIENTE: CÓDIGO DA CLIENTE não informado');
...
procedure TCLIENTE_TELEFONES.Atualizar(const AFDQ_Query: TFDQuery; AID_CLIENTE:Integer = 0; AID:Integer = 0);
begin
   try
     try
       AFDQ_Query.Connection := FConexao;
...
procedure TCLIENTE_TELEFONES.Excluir(const AFDQ_Query:TFDQuery; AID_CLIENTE:Integer = 0; AID:Integer = 0);
begin
  try
    try
      AFDQ_Query.Connection := FConexao;
...
procedure TCLIENTE_TELEFONES.SetID_CLIENTE( const Value:Integer);
begin
  FID_CLIENTE := Value;
end;

procedure TCLIENTE_TELEFONES.SetID( const Value:Integer);
begin
  FID := Value;
end;

procedure TCLIENTE_TELEFONES.SetTIPO( const Value:Integer);
begin
  FTIPO := Value;
end;

procedure TCLIENTE_TELEFONES.SetNUMERO( const Value:String);
begin
  FNUMERO := Value;
end;

procedure TCLIENTE_TELEFONES.SetID_USUARIO( const Value:Integer);
begin
  FID_USUARIO := Value;
end;

procedure TCLIENTE_TELEFONES.SetDT_CADASTRO( const Value:TDate);
begin
  FDT_CADASTRO := Value;
end;

procedure TCLIENTE_TELEFONES.SetHR_CADASTRO( const Value:TTime);
begin
  FHR_CADASTRO := Value;
end;
