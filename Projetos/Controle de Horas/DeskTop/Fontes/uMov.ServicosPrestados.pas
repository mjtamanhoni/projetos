...
type
  ...
  TfrmMov_ServicosPrestados = class(TForm)
    ...
    FDQRegistroshr_total: TWideStringField;
    FDQRegistrosvlr_hora: TBCDField;
    FDQRegistrossub_total: TBCDField;
    FDQRegistrosdesconto: TBCDField;
    FDQRegistrosdesconto_motivo: TWideStringField;
    FDQRegistrosacrescimo: TBCDField;
    FDQRegistrosacrescimo_motivo: TWideStringField;
    FDQRegistrostotal: TBCDField;
    FDQRegistrosobservacao: TWideStringField;
    FDQRegistrosdt_pago: TDateField;
    FDQRegistrosvlr_pago: TBCDField;
    FDQRegistrosdt_cadastro: TDateField;
    FDQRegistroshr_cadastro: TTimeField;
    FDQRegistrosid_usuario: TIntegerField;
    FDQRegistrosstatus_desc: TWideMemoField;
    FDQRegistrosempresa: TWideStringField;
    FDQRegistrosprestaor_servico: TWideStringField;
    FDQRegistroscliente: TWideStringField;
    FDQRegistrosconta: TWideStringField;
    FDQRegistrostipo: TIntegerField;
    FDQRegistrostipo_conta: TWideMemoField;
    FDQRegistrostabela: TWideStringField;
    FDQRegistrostabela_tipo: TWideMemoField;
    FDQRegistrosvalor: TBCDField;
    FDQRegistrossegundos: TIntegerField;
...
begin
  try
    try
      ...
      FQuery.Active := False;
      FQuery.SQL.Clear;
      FQuery.SQL.Add('select nextval(''servicos_prestados_id_seq'') as seq;');
      FQuery.Active := True;
      if not FQuery.IsEmpty then
        FId := FQuery.FieldByName('seq').AsInteger;
...
begin
  try
    try
      ...
      FDQRegistros.SQL.Add('  sp.id_empresa ');
      FDQRegistros.SQL.Add('  ,sp.id_prestador_servico ');
      FDQRegistros.SQL.Add('  ,sp.id_conta ');
      FDQRegistros.SQL.Add('  ,sp."DATA" ');
      FDQRegistros.SQL.Add('  ,sp.hr_inicio; ');
      ...
      {$Region 'Totalizando'}
        ...
        FDQ_Total.Sql.Add('  ,concat(lpad(cast(cast(extract(epoch from d.dh::timestamp - current_date::timestamp) / 3600 as integer) as text),3,''0''), '':'', ');
        FDQ_Total.Sql.Add('          lpad(cast(cast(extract(minute from d.dh) as integer) as text),2,''0''), '':'', ');
        FDQ_Total.Sql.Add('          lpad(cast(cast(extract(second from d.dh) as integer) as text),2,''0'')) as hora ');
        FDQ_Total.Sql.Add('  ,d.total ');
        FDQ_Total.Sql.Add('from (select ');
        FDQ_Total.Sql.Add('        c.tipo ');
        FDQ_Total.Sql.Add('        ,(c.dm + interval ''1 hours'' * c.hora) as dh ');
        FDQ_Total.Sql.Add('        ,c.total ');
        FDQ_Total.Sql.Add('      from (select ');
        FDQ_Total.Sql.Add('              b.tipo ');
        FDQ_Total.Sql.Add('              ,b.hora ');
        FDQ_Total.Sql.Add('              ,(b.ds + interval ''1 minutes'' * b.minuto) as dm ');
        FDQ_Total.Sql.Add('              ,b.total ');
        FDQ_Total.Sql.Add('            from (select ');
        FDQ_Total.Sql.Add('                    a.tipo ');
        FDQ_Total.Sql.Add('                    ,a.hora ');
        FDQ_Total.Sql.Add('                    ,a.minuto ');
        FDQ_Total.Sql.Add('                    ,(current_date + interval ''1 seconds'' * a.segundo) as ds ');
