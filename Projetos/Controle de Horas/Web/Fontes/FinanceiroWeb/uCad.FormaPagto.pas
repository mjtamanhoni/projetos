...
begin
  ...
  {$Region 'Inserindo a Condição de pagamento'}
    if FinanceiroWeb.Cond_Pagto_ID > 0 then
    begin
      ...
      begin
        ...
        begin
          if ((FDMem_CondicaoID_FORMA_PAGAMENTO.AsInteger = FDMem_RegistroID.AsInteger) and
              (FDMem_CondicaoID_CONDICAO_PAGAMENTO.AsInteger = FinanceiroWeb.Cond_Pagto_ID)) then
          begin
            FInserir := False;
          end;
        end;
  ...
  try
    try
     ...
     FDMem_FormaCond_Pagto.Insert;
      ...
      FDMem_FormaCond_PagtoID_CONDICAO_PAGAMENTO.AsInteger := FinanceiroWeb.Cond_Pagto_ID;
...
begin
  FinanceiroWeb.Cond_Pagto_ID := 0;
  FinanceiroWeb.Cond_Pagto_Nome := '';
  if not FDMem_Registro.IsEmpty then
  begin
    FinanceiroWeb.Cond_Pagto_ID := FDMem_RegistroID.AsInteger;
    FinanceiroWeb.Cond_Pagto_Nome := FDMem_RegistroDESCRICAO.AsString;
  end;
  Action := CaFree;
