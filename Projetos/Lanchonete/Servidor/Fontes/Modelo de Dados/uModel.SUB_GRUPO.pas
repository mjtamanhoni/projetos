...
type 
  ...
  public 
    ...
    function Listar(const AFDQ_Query:TFDQuery; AID_GRUPO:Integer = 0; AID:Integer = 0): TJSONArray; 
    procedure Atualizar(const AFDQ_Query:TFDQuery; AID_GRUPO:Integer = 0; AID:Integer = 0); 
    procedure Excluir(const AFDQ_Query:TFDQuery; AID_GRUPO:Integer = 0; AID:Integer = 0); 
...
constructor TSUB_GRUPO.Create(AConnexao: TFDConnection); 
begin 
  FConexao := AConnexao; 
end; 
 
destructor TSUB_GRUPO.Destroy; 
begin 
 
  inherited; 
end; 
 
procedure TSUB_GRUPO.Criar_Estrutura(const AFDScript: TFDScript; AFDQuery: TFDQuery); 
var 
  lScript: TStringList; 
begin 
  try 
    try 
      ...
      lScript := TStringList.Create; 
       ...
       begin 
         ...
         begin 
           ...
           begin 
            //Excluindo objetos da tabela SUB_GRUPO... 
            if Indice_Existe(FConexao,AFDQuery,'FK_SUB_GRUPO_1') then 
              Add('DROP INDEX FK_SUB_GRUPO_1;'); 
 
            if Trigger_Existe(FConexao,AFDQuery,'SUB_GRUPO_BI') then 
              Add('DROP TRIGGER SUB_GRUPO_BI;'); 
 
 
            if Tabela_Existe(FConexao,AFDQuery,'SUB_GRUPO') then 
              Add('DROP TABLE SUB_GRUPO;'); 
     ...
     //Criando tabelas...
      lScript.Clear; 
      lScript.Add('CREATE TABLE SUB_GRUPO ( '); 
      ...
      begin 
        ...
        with SQLScripts[0].SQL do 
        begin 
          Add('ALTER TABLE SUB_GRUPO ADD CONSTRAINT PK_SUB_GRUPO PRIMARY KEY (ID_GRUPO,ID);');
          Add('ALTER TABLE SUB_GRUPO ADD CONSTRAINT FK_SUB_GRUPO_1 FOREIGN KEY (ID_GRUPO) REFERENCES GRUPO(ID) ON DELETE CASCADE;');
          Add('CREATE INDEX FK_SUB_GRUPO_1 ON SUB_GRUPO (ID_GRUPO);');
          Add('COMMENT ON COLUMN SUB_GRUPO.ID_GRUPO IS ''CODIGO DO GRUPO'';');
          Add('COMMENT ON COLUMN SUB_GRUPO.ID IS ''SEQUENCIAL'';');
          Add('COMMENT ON COLUMN SUB_GRUPO.NOME IS ''NOME'';');
          Add('COMMENT ON COLUMN SUB_GRUPO.ID_USUARIO IS ''CODIGO DO USUARIO'';');
          Add('COMMENT ON COLUMN SUB_GRUPO.DT_CADASTRO IS ''DATA DO CADASTRO'';');
          Add('COMMENT ON COLUMN SUB_GRUPO.HR_CADASTRO IS ''HORA DO CADASTRO'';');
          ValidateAll; 
          ExecuteAll; 
        end; 
      end; 
      ...
      lScript.Add('CREATE OR ALTER TRIGGER SUB_GRUPO_BI FOR SUB_GRUPO '); 
      lScript.Add('ACTIVE BEFORE INSERT POSITION 0');
      lScript.Add('as');
      lScript.Add('begin');
      lScript.Add('  if (new.id is null) then');
      lScript.Add('    SELECT COALESCE(MAX(ID),0) + 1 FROM SUB_GRUPO WHERE ID_GRUPO = NEW.ID_GRUPO INTO new.id;');
      lScript.Add('end ^');
      lScript.Add(' '); 
      AFDScript.ExecuteScript(lScript); 
    except 
...
procedure TSUB_GRUPO.Atualizar_Estrutura(const AFDQ_Query: TFDQuery); 
begin 
 
end; 
 
procedure TSUB_GRUPO.Inicia_Propriedades; 
begin 
  ID_GRUPO := -1; 
  ID := -1; 
  NOME := ''; 
...
function  TSUB_GRUPO.Listar(const AFDQ_Query:TFDQuery; AID_GRUPO:Integer = 0; AID:Integer = 0): TJSONArray;
begin
  try
    try
      ...
      AFDQ_Query.Active := False;
      AFDQ_Query.Sql.Clear;
      AFDQ_Query.Sql.Add('SELECT * FROM SUB_GRUPO');
...
procedure TSUB_GRUPO.Inserir(const AFDQ_Query: TFDQuery); 
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('INSERT INTO SUB_GRUPO( ');
...
procedure TSUB_GRUPO.Atualizar(const AFDQ_Query: TFDQuery; AID_GRUPO:Integer = 0; AID:Integer = 0);
begin 
   try 
     try 
       AFDQ_Query.Connection := FConexao; 
      ...
      AFDQ_Query.Sql.Add('UPDATE SUB_GRUPO SET ');
      if FNOME <> '' then 
      ...
      AFDQ_Query.Sql.Add('  AND ID_GRUPO = :ID_GRUPO');
...
procedure TSUB_GRUPO.Excluir(const AFDQ_Query:TFDQuery; AID_GRUPO:Integer = 0; AID:Integer = 0);
begin 
  try 
    try 
      AFDQ_Query.Connection := FConexao; 
      AFDQ_Query.Active := False; 
      AFDQ_Query.Sql.Clear; 
      AFDQ_Query.Sql.Add('DELETE FROM SUB_GRUPO '); 
      AFDQ_Query.Sql.Add('WHERE NOT ID IS NULL '); 
      AFDQ_Query.Sql.Add('  AND ID_GRUPO = :ID_GRUPO');
      AFDQ_Query.ParamByName('ID_GRUPO').AsInteger := AID_GRUPO;
      AFDQ_Query.Sql.Add('  AND ID = :ID');
...
procedure TSUB_GRUPO.SetID_GRUPO( const Value:Integer);
begin
  FID_GRUPO := Value;
end;
 
procedure TSUB_GRUPO.SetID( const Value:Integer);
begin 
  FID := Value; 
end;
 
procedure TSUB_GRUPO.SetNOME( const Value:String);
begin 
  FNOME := Value; 
end;
 
procedure TSUB_GRUPO.SetID_USUARIO( const Value:Integer);
begin 
  FID_USUARIO := Value; 
end;
 
procedure TSUB_GRUPO.SetDT_CADASTRO( const Value:TDate);
begin 
  FDT_CADASTRO := Value; 
end;
 
procedure TSUB_GRUPO.SetHR_CADASTRO( const Value:TTime);
begin 
  FHR_CADASTRO := Value; 
end;
