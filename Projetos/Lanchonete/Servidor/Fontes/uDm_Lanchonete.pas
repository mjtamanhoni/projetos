...
type
  TDM_Lanchonete = class(TDataModule)
    FDC_Lanchonete: TFDConnection;
    FDT_Lanchonete: TFDTransaction;
    FDP_Lanchonete: TFDPhysFBDriverLink;
...
procedure TDM_Lanchonete.DataModuleCreate(Sender: TObject);
var
  lEnder_Ini :String;
...
begin
  ...
  FDC_Lanchonete.Connected := False;
  ...
  try
    {$Region 'Conectando banco de dados Firebird'}
      lDatabase := '';
      lUser_Name := '';
      lPassword := '';
      lProtocol := '';
      ...
      FDC_Lanchonete.LoginPrompt := False;
      FDC_Lanchonete.Params.Clear;
      FDC_Lanchonete.Params.Add('Database=' + lDatabase);
      FDC_Lanchonete.Params.Add('User_Name=' + lUser_Name);
      FDC_Lanchonete.Params.Add('Password=' + lPassword);
      FDC_Lanchonete.Params.Add('Protocol=' + lProtocol);
      FDC_Lanchonete.Params.Add('Port=' + lPort);
      FDC_Lanchonete.Params.Add('Server=' + lServer);
      FDC_Lanchonete.Params.Add('DriverID=' + lDriverID);

      if lVersao = 0 then
      begin
        FDP_Lanchonete.VendorLib := '';
        FDP_Lanchonete.VendorLib := lBiblioteca;
      end;

    {$EndRegion 'Conectando banco de dados Firebird'}

    FDC_Lanchonete.Connected := True;
  except
    On Ex:Exception do
      Firebird_Erro := Ex.Message;
  end;

  Firebird_Conectado := FDC_Lanchonete.Connected;
end;

procedure TDM_Lanchonete.DataModuleDestroy(Sender: TObject);
begin
  {$IFDEF MSWINDOWS}
    FreeAndNil(FIniFile);
  {$ELSE}
...
procedure TDM_Lanchonete.Listar_Tabelas(const AQuery:TFDQuery;const ATabela:String='');
begin
  try
    try
      AQuery.Connection := FDC_Lanchonete;
      AQuery.Active := False;
      AQuery.Sql.Clear;
...
procedure TDM_Lanchonete.Listar_Triggers(const AQuery: TFDQuery; const ATabela,
  ATrigger: String);
begin
  try
    try
...
procedure TDM_Lanchonete.Listar_Campos(const AQuery:TFDQuery;const ATabela:String='';const ACampo:String='');
begin
  try
    try
      AQuery.Connection := FDC_Lanchonete;
...
procedure TDM_Lanchonete.Listar_Filtros(const AQuery: TFDQuery;
...
procedure TDM_Lanchonete.Listar_ForeingKey(const AQuery: TFDQuery;
  const ATabela, AForeingKey: String);
begin
  try
    try
      AQuery.Connection := FDC_Lanchonete;
...
procedure TDM_Lanchonete.Listar_Generator(const AQuery: TFDQuery; const ATabela,
  AGenerator: String);
begin
  try
    try
...
procedure TDM_Lanchonete.Listar_Indices(const AQuery: TFDQuery; const ATabela,
  AIndice: String);
begin
  try
    try
...
procedure TDM_Lanchonete.Listar_PrimaryKey(const AQuery: TFDQuery;
  const ATabela, APrimaryKey: String);
begin
  try
    try
      AQuery.Connection := FDC_Lanchonete;
...
procedure TDM_Lanchonete.Refazendo_Estrutura;
var
  lTUsuario :TUsuario;
...
begin
  try
    ...
    {$IFDEF MSWINDOWS}
      ...
      FreeAndNil(FDS_Script);
    {$ELSE}
      lTUsuario.DisposeOf;
      lTUsuario_Permissoes.DisposeOf;
      lTFornecedor.DisposeOf;
      lTFornecedor_Telefone.DisposeOf;
      FDQ_Query.DisposeOf;
      FDS_Script.DisposeOf;
    {$ENDIF}
  end;
