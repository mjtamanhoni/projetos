...
procedure TfrmCadSetor.Abortar_Fechamento(Sender: TOBject);
begin
  Abort;
end;

procedure TfrmCadSetor.Cancelar_Alteracoes(Sender: TOBject);
begin
  Configura_Botoes(2);
end;

procedure TfrmCadSetor.Configura_Botoes(ABotao: Integer);
begin
  case ABotao of
    0:begin
      //Novo...
...
procedure TfrmCadSetor.Confirmar_Fechamento(Sender: TObject);
begin
...
procedure TfrmCadSetor.Deletar_Registro(Sender: TOBject);
var
  t :TThread;
begin
  ...
  procedure
  begin
    Dm_DeskTop.Setor_Excluir(FId_Selecionado);
...
procedure TfrmCadSetor.EditandoRegistro(const AId: Integer);
var
  t :TThread;
  lNome :String;
  lCodigo :Integer;
...
begin
  ...
  var
    ...
    lHr_Cadastro :TTime;
  begin
      jsonArray := Dm_DeskTop.Setor_Lista(
        0
        ,AId
        ,'');
...
procedure TfrmCadSetor.EditarRegistro(Sender: TOBject);
begin
  EditandoRegistro(FId_Selecionado);
end;

procedure TfrmCadSetor.edNomeTyping(Sender: TObject);
begin
  TFuncoes.ExibeLabel(edNome,lbNome,faNome,10,-20);
end;

procedure TfrmCadSetor.edPesquisarKeyDown(Sender: TObject; var Key: Word;
  var KeyChar: Char; Shift: TShiftState);
begin
...
procedure TfrmCadSetor.edPesquisarTyping(Sender: TObject);
begin
  TFuncoes.ExibeLabel(edPesquisar,lbPesquisar,FloatAnimation_Pesq,10,-20);
end;

procedure TfrmCadSetor.Exibe_Labels;
begin
  TFuncoes.ExibeLabel(edNome,lbNome,faNome,10,-20);
end;
...
begin
  ...
  FMensagem := TFancyDialog.Create(frmCadSetor);
...
procedure TfrmCadSetor.Incia_Campos;
begin
  edNome.Tag := 0;
  edNome.Text := '';
  Exibe_Labels;
...
procedure TfrmCadSetor.Listar_Dados(const APagina: Integer;
...
procedure TfrmCadSetor.AddItens_LV(
        const ACodigo:Integer;
        const ANome:String);
begin
...
procedure TfrmCadSetor.ThreadEnd_Lista(Sender: TOBject);
begin
  if Assigned(TThread(Sender).FatalException) then
  begin
    if Pos('401',Exception(TThread(Sender).FatalException).Message) = 0 then
      FMensagem.Show(TIconDialog.Error,'','Erro na carga dos Setores: ' + Exception(TThread(Sender).FatalException).Message);
  end;
end;

procedure TfrmCadSetor.lvListaItemClick(const Sender: TObject;
  const AItem: TListViewItem);
begin
...
procedure TfrmCadSetor.lvListaPaint(Sender: TObject; Canvas: TCanvas;
  const ARect: TRectF);
begin
...
procedure TfrmCadSetor.Novo_Registro(Sender: TOBject);
begin
  Configura_Botoes(0);
end;

procedure TfrmCadSetor.rctCancelarClick(Sender: TObject);
begin
...
procedure TfrmCadSetor.rctConfirmarClick(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    if FStatusTable = stInsert then
    begin
      if not Dm_DeskTop.Setor_Cadastro(FDMem_Registros.ToJSONArray,0) then
        raise Exception.Create('Erro ao salvar as alterações');
    end
    else if FStatusTable = stUpdate then
    begin
      if not Dm_DeskTop.Setor_Cadastro(FDMem_Registros.ToJSONArray,1) then
        raise Exception.Create('Erro ao salvar as alterações');
    end;
...
procedure TfrmCadSetor.SetGuia_Inicial(const Value: Integer);
begin
  FGuia_Inicial := Value;
end;

procedure TfrmCadSetor.ThreadEnd_DeletarRegistro(Sender: TOBject);
begin
  if Assigned(TThread(Sender).FatalException) then
...
procedure TfrmCadSetor.ThreadEnd_Edit(Sender: TOBject);
begin
  if Assigned(TThread(Sender).FatalException) then
...
procedure TfrmCadSetor.ThreadEnd_SalvarRegistro(Sender: TOBject);
begin
  if Assigned(TThread(Sender).FatalException) then
    FMensagem.Show(TIconDialog.Error,'Erro',Exception(TThread(Sender).FatalException).Message)
  else
