...
type
  ...
  TfrmCad_Cliente = class(TForm)
    ...
    imgVoltar: TImage;
    lytRow_012: TLayout;
    lytID_FORNECEDOR: TLayout;
    lbID_FORNECEDOR: TLabel;
    edID_FORNECEDOR: TEdit;
    imgID_FORNECEDOR: TImage;
    lytRow_013: TLayout;
    lytID_TAB_PRECO: TLayout;
    ...
    procedure edID_FORNECEDORKeyDown(Sender: TObject; var Key: Word; var KeyChar: Char; Shift: TShiftState);
    ...
    procedure edID_FORNECEDORClick(Sender: TObject);
    procedure edID_TAB_PRECOClick(Sender: TObject);
...
procedure TfrmCad_Cliente.Abre_Menu_Registros(Sender: TOBject);
var
...
procedure TfrmCad_Cliente.AddRegistros_LB(AId, ASincronizado, AExcluido: Integer; ANome, ADocumento,
  ACeluar: String);
var
  FItem :TListBoxItem;
  FFrame :TFrame_Empresa;
...
procedure TfrmCad_Cliente.Cancelar(Sender: TOBject);
begin
  FTab_Status := dsLista;
  LimparCampos;
  imgAcao_01.Tag := 0;
...
procedure TfrmCad_Cliente.CriandoCombos;
begin

  cComboUF := TCustomCombo.Create(frmCad_Cliente);
  cComboPessoa := TCustomCombo.Create(frmCad_Cliente);
...
procedure TfrmCad_Cliente.CriandoMenus;
begin
  FMenu_Frame := TActionSheet.Create(frmCad_Cliente);
...
procedure TfrmCad_Cliente.edID_FORNECEDORClick(Sender: TObject);
begin
  ...
  frmCad_Fornecedor.ExecuteOnClose := Sel_Fornecedor;
  frmCad_Fornecedor.Show;

end;

procedure TfrmCad_Cliente.edID_FORNECEDORKeyDown(Sender: TObject; var Key: Word; var KeyChar: Char;
  Shift: TShiftState);
begin
  if Key = vkReturn then
...
procedure TfrmCad_Cliente.edID_TAB_PRECOClick(Sender: TObject);
begin
  ...
  frmCad_TabelaPreco.ExecuteOnClose := Sel_TabPrecos;
...
procedure TfrmCad_Cliente.Editar(Sender: TOBject);
var
  t :TThread;
begin
  FMenu_Frame.HideMenu;
  TLoading.Show(frmCad_Cliente,'Editando o Registro');
  LimparCampos;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        edID_FORNECEDOR.Tag := FQuery.FieldByName('ID_FORNECEDOR').AsInteger;
        with FFornecedor.Lista_Registros(edID_FORNECEDOR.Tag) do
        begin
          if not IsEmpty then
          begin
            edID_FORNECEDOR.Text := FieldByName('NOME').AsString;
            {$IFDEF MSWINDOWS}
              Free;
            {$ELSE}
              DisposeOf;
        ...
        edID_TAB_PRECO.Tag := FQuery.FieldByName('ID_TAB_PRECO').AsInteger;
        with FTab_Preco.Lista_Registros(edID_TAB_PRECO.Tag) do
        begin
          if not IsEmpty then
          begin
            edID_TAB_PRECO.Text := FieldByName('DESCRICAO').AsString;
            {$IFDEF MSWINDOWS}
              Free;
            {$ELSE}
              DisposeOf;
...
procedure TfrmCad_Cliente.edPESSOAClick(Sender: TObject);
begin
...
procedure TfrmCad_Cliente.edUFClick(Sender: TObject);
begin
  cComboUF.ShowMenu;
end;
...
procedure TfrmCad_Cliente.Excluir_Registro(Sender: TObject);
var
  t :TThread;
begin
  TLoading.Show(frmCad_Cliente,'Excluindo registro');
  ...
  begin
    ...
    FQuery.Active := False;
    FQuery.Sql.Clear;
    FQuery.Sql.Add('UPDATE CLIENTE SET EXCLUIDO = 1 WHERE ID = :ID ');
    FQuery.ParamByName('ID').AsInteger := FId;
    FQuery.ExecSQL;;
  end);
...
begin
  ...
  FFornecedor := TFornecedor.Create(FDm_Global.FDC_SQLite,FEnder);
  FTab_Preco := TTab_Preco.Create(FDm_Global.FDC_SQLite,FEnder);
...
procedure TfrmCad_Cliente.imgAcao_01Click(Sender: TObject);
begin
...
procedure TfrmCad_Cliente.imgAvancar_001Click(Sender: TObject);
begin
  tcCampos.GotoVisibleTab(1);
end;

procedure TfrmCad_Cliente.imgAvancar_002Click(Sender: TObject);
begin
  tcCampos.GotoVisibleTab(2);
end;

procedure TfrmCad_Cliente.imgRetornar_001Click(Sender: TObject);
begin
  tcCampos.GotoVisibleTab(0);
end;

procedure TfrmCad_Cliente.imgRetornar_002Click(Sender: TObject);
begin
  tcCampos.GotoVisibleTab(1);
end;

procedure TfrmCad_Cliente.imgVoltarClick(Sender: TObject);
begin
...
{$IFDEF MSWINDOWS}
procedure TfrmCad_Cliente.ItemClick_Pessoa(Sender: TObject);
begin
...
{$ELSE}
procedure TfrmCad_Cliente.ItemClick_Pessoa(Sender: TObject; const Point: TPointF);
begin
  cComboPessoa.HideMenu;
  edPessoa.Text := cComboPessoa.DescrItem;
  edPessoa.Tag := StrToIntDef(cComboPessoa.CodItem,0);
...
{$IFDEF MSWINDOWS}
procedure TfrmCad_Cliente.ItemClick_UF(Sender: TObject);
begin
  cComboUF.HideMenu;
  edUF.Text := cComboUF.CodItem;
end;
{$ELSE}
procedure TfrmCad_Cliente.ItemClick_UF(Sender: TObject; const Point: TPointF);
begin
  cComboUF.HideMenu;
  edUF.Text := cComboUF.CodItem;
...
procedure TfrmCad_Cliente.lbRegistrosItemClick(const Sender: TCustomListBox; const Item: TListBoxItem);
begin
  FId := Item.Tag;
  FNome := Item.TagString;
end;

procedure TfrmCad_Cliente.LimparCampos;
begin
  edID.Text := '';
  edID.Tag := 0;
  edPESSOA.Text := '';
  ...
  edID_FORNECEDOR.Tag := 0;
  edID_TAB_PRECO.Text := '';
  edID_TAB_PRECO.Tag := 0;
end;

procedure TfrmCad_Cliente.Listar_Registros(APesquisa: String);
var
  t :TThread;
begin
  TLoading.Show(frmCad_Cliente,'Listando Registros');
  lbRegistros.Clear;
  ...
  begin
    ...
    begin
      FQuery.Sql.Add('  AND T.NOME LIKE :NOME');
...
var
  t :TThread;
begin
  TLoading.Show(frmCad_Cliente,'Salvando alterações');
  ...
  begin
    ...
    FQuery.ParamByName('ID_FORNECEDOR').AsInteger := edID_FORNECEDOR.Tag;
    FQuery.ParamByName('ID_TAB_PRECO').AsInteger := edID_TAB_PRECO.Tag;
    FQuery.ExecSQL;
...
procedure TfrmCad_Cliente.Sel_Fornecedor(Aid: Integer; ANome: String);
begin
  edID_FORNECEDOR.Tag := Aid;
...
procedure TfrmCad_Cliente.TThreadEnd_Editar(Sender: TOBject);
begin
  TLoading.Hide;
...
procedure TfrmCad_Cliente.TThreadEnd_ExcluirRegistro(Sender: TOBject);
begin
  TLoading.Hide;
  if Assigned(TThread(Sender).FatalException) then
    FFancyDialog.Show(TIconDialog.Error,'Erro',Exception(TThread(Sender).FatalException).Message)
...
procedure TfrmCad_Cliente.TThreadEnd_Listar_Registros(Sender: TObject);
begin
  TLoading.Hide;
  if Assigned(TThread(Sender).FatalException) then
    FFancyDialog.Show(TIconDialog.Error,'Erro',Exception(TThread(Sender).FatalException).Message);
...
procedure TfrmCad_Cliente.TTHreadEnd_Salvar(Sender: TOBject);
begin
  TLoading.Hide;
