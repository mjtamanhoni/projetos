...
type
  TfrmProjetos_Loc = class(TD2BridgeForm)
...
function frmProjetos_Loc:TfrmProjetos_Loc;
...
function frmProjetos_Loc:TfrmProjetos_Loc;
begin
  result:= TfrmProjetos_Loc(TfrmProjetos_Loc.GetInstance);
end;

procedure TfrmProjetos_Loc.btCancelarClick(Sender: TObject);
begin
  {$Region 'Retornando valores'}
    Gestao_Financeira.Projeto_Id := 0;
    Gestao_Financeira.Projeto_Descricao := '';
    Gestao_Financeira.Projeto_Status := -1;
  {$Region 'Retornando valores'}
...
procedure TfrmProjetos_Loc.btConfirmarClick(Sender: TObject);
begin
  {$Region 'Retornando valores'}
    Gestao_Financeira.Projeto_Id := FDMem_Registroid.AsInteger;
    Gestao_Financeira.Projeto_Status := FDMem_Registrostatus.AsInteger;
...
procedure TfrmProjetos_Loc.edPesquisarRightButtonClick(Sender: TObject);
begin
  Pesquisar;
end;

procedure TfrmProjetos_Loc.ExportD2Bridge;
begin
  inherited;

  Title:= 'Localiza Projeto';
...
procedure TfrmProjetos_Loc.InitControlsD2Bridge(const PrismControl: TPrismControl);
begin
 inherited;
...
procedure TfrmProjetos_Loc.Pesquisar;
var
  FResp :IResponse;
  FBody :TJSONArray;
...
procedure TfrmProjetos_Loc.RenderD2Bridge(const PrismControl: TPrismControl; var HTMLControl: string);
begin
 inherited;
