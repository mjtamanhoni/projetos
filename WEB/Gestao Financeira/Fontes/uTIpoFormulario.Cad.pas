    ...
    try
      ...
      {$Region 'Gravando/Gerando JSon dos dados'}
        ...
        if ((Gestao_Financeira.Projeto_Status_Tab = 'Edit') and (Trim(edid.Text) = ''))  then
          raise Exception.Create('Id do Tipo de Formulário é obrigatório');

        if Trim(eddescricao.Text) = '' then
          raise Exception.Create('Nome do Tipo de Formulário é obrigatório');
...
begin
  ...
  Title:= 'Cadastro de Tipo de Formulário';
